<template>
  <div class="app-container"
    ...
    <custom-config ref="customConfig"></custom-config>
  </div>
</template>
<script>
...
const url = '/basic/customConfigDef/selects'
export default {
  name: 'customConfigDefIndex',
  components: {
    SearchForm,
    BasicTable,
    UpdateForm,
  ...
  methods: {
    ...
    getAddForm() {
      ...
      baseRequest('/basic/customConfigDef/getAddData', param).then(response => {
        response.data.item = {}
        const result = response.data
        result.formConfig = JSON.parse(result.formConfig)
    ...
    getUpdateForm(row) {
      ...
      baseRequest('/basic/customConfigDef/select', param).then(response => {
        const result = response.data
        result.formConfig = JSON.parse(result.formConfig)
        this.updateFormData = result
        this.formVisible = true
    ...
    deleteForm(row) {
      ...
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        baseRequest('/basic/customConfigDef/delete', param).then(response => {
          this.searchOption()
    ...
    saveOperate() {
      if (this.operateStatus === 1) {
        this.$refs.updateFrom.saveRequest('/customConfigDef/add').then(() => {
          this.searchOption()
          this.$Message.success('操作成功')
        ...
        this.$refs.updateFrom.saveRequest('/customConfigDef/update', param).then(() => {
          this.searchOption()
          this.$Message.success('操作成功')
          this.formVisible = false
        })
    ...
    seeView(row) {
      this.$refs.customConfig.openDialog(row)
    }
  }
}
</script>
