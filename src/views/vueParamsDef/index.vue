...
const url = '/basic/vueParamsDef/selects'
export default {
  name: 'vueParamsDefIndex',
  components: {
    SearchForm,
    BasicTable,
    UpdateForm
  ...
  methods: {
    ...
    getAddForm() {
      ...
      baseRequest('/basic/vueParamsDef/getAddData', param).then(response => {
        response.data.item = {}
        const result = response.data
        result.formConfig = JSON.parse(result.formConfig)
    ...
    getUpdateForm(row) {
      ...
      baseRequest('/basic/vueParamsDef/select', param).then(response => {
        const result = response.data
        result.formConfig = JSON.parse(result.formConfig)
        this.updateFormData = result
        this.formVisible = true
    ...
    deleteForm(row) {
      ...
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        baseRequest('/basic/vueParamsDef/delete', param).then(response => {
          this.searchOption()
    ...
    saveOperate() {
      if (this.operateStatus === 1) {
        this.$refs.updateFrom.saveRequest('/vueParamsDef/add').then(() => {
          this.searchOption()
          this.$Message.success('操作成功')
        ...
        this.$refs.updateFrom.saveRequest('/vueParamsDef/update', param).then(() => {
          this.searchOption()
          this.$Message.success('操作成功')
          this.formVisible = false
        })
