...
export default {
  ...
  methods: {
    ...
    getAddForm() {
      ...
      baseRequest('/configs/getAddData', param).then(response => {
        response.data.item = {}
        const result = response.data
        result.formConfig = JSON.parse(result.formConfig)
    ...
    getUpdateForm(row) {
      ...
      baseRequest('/configs/select', param).then(response => {
        const result = response.data
        result.formConfig = JSON.parse(result.formConfig)
        this.updateFormData = result
        this.formVisible = true
    ...
    deleteForm(row) {
      ...
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        baseRequest('/configs/delete', param).then(response => {
          this.searchOption()
    ...
    saveOperate() {
      if (this.operateStatus === 1) {
        this.$refs.updateFrom.saveRequest('/configs/add').then(() => {
          this.searchOption()
          this.$Message.success('操作成功')
        ...
        this.$refs.updateFrom.saveRequest('/configs/update', param).then(() => {
          this.searchOption()
          this.$Message.success('操作成功')
          this.formVisible = false
        })
